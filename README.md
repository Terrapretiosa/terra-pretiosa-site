...
> ⚠️ The production domain `https://www.terrapretiosa.com` is hardcoded in three files:
> `src/app/layout.tsx`, `src/app/sitemap.ts`, and `src/app/robots.ts`.
> **If the domain ever changes, all three must be updated together.**
> Note the `www.`: the apex `terrapretiosa.com` answers with a 308 redirect to it,
> so it is the `www` host that actually serves the site.
